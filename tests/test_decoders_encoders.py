...
def int_to_bits(n, width):
    return tuple(int(bool((n >> i) & 1)) for i in range(width))
...
def test_decoder2x4_exhaustive():
    for i in range(4):
        ...
        assert len(out) == 4
        assert sum(bit for bit in out) == 1, f"Not one-hot: {out}"
...
def test_decoder3x8_exhaustive():
    for i in range(8):
        ...
        assert len(out) == 8
        assert sum(bit for bit in out) == 1
...
def test_encoder8x3_exhaustive_one_hot():
    ...
    for i in range(8):
        ...
        encoded = (out[0] << 0) | (out[1] << 1) | (out[2] << 2)
...
def test_decoder3x8_encoder8x3_round_trip():
    ...
    for i in range(1, 8):
        ...
        encoded = (enc[0] << 0) | (enc[1] << 1) | (enc[2] << 2)
        assert encoded == i
...
def test_power_signal_variations():
    """
    Ensure that power=True or custom 1 does not break behavior.
    """
    for i in range(4):
        b0, b1 = int_to_bits(i, 2)
        out = decoder2x4(b0, b1, 1)
